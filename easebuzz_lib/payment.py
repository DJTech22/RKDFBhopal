...
def initiate_payment(params, merchant_key, salt, env):
    try:
        print("merchant_keymerchant_keymerchant_key",merchant_key)
        result = _payment(params, merchant_key, salt, env)
        print("initiate_payment  ----- initiate_payment ",result )
...
def _removeSpaceAndPreparePostArray(params):
    ...
    temp_distionary = {
      ...
      'FatherName' : params['FatherName'].strip(),
    ...
    print("__---------temp_distionary-------",temp_distionary)
...
def _typeValidation(params):
    ...
    if not(isinstance(float(params['amount']), float)):
        print("Arjunnn**********************************Amount",type(params['amount']))
...
def _getHashKey(posted, salt_key):
    ...
    print("******534********",sha512(hash_string.encode('utf-8')).hexdigest().lower())
